...
(function( $ ) {
	...
	function initNavigation( containerClass, naviClass ) {
		...
		(function() {
			...
			menuToggle.on( 'click.ovi_', function() {
		...
		container.find( '.item.dropdown.active > .sub-menu' ).addClass( 'toggled-on' );
			...
			navigation.find( '.dropdown-toggle-button' ).click( function( e ) {
				...
				screenReaderSpan.text( screenReaderSpan.text() === oviScreenReaderText.expand ? oviScreenReaderText.collapse : oviScreenReaderText.expand );
			} );
		})();
		...
		(function() {
			...
			function toggleFocusClassTouchScreen() {
				if ( 'none' === $( '.menu-toggle' ).css( 'display' ) ) {
					...
					$( document.body ).on( 'touchstart.ovi_', function( e ) {
					...
					menuList.find( '.menu-item-has-children > a, .page_item_has_children > a' ).unbind( 'touchstart.ovi_' );
				}
			}

			if ( 'ontouchstart' in window ) {
				$( window ).on( 'resize.ovi_', toggleFocusClassTouchScreen );
				toggleFocusClassTouchScreen();
			}

			menuList.find( 'a' ).on( 'focus.ovi_ blur.ovi_', function() {
